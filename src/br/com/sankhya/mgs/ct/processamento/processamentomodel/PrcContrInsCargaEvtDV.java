...
public class PrcContrInsCargaEvtDV extends ProcessarSuper implements Processar {

    public PrcContrInsCargaEvtDV() {
        super();
    }
        ...
        try {
            ...
            if (!integracaoSalva) {
                ...
                if (BigDecimal.ONE.equals(sucesso)) {
                    ...
                    executado = false;
                    mensagem = "Erro PrcContrInsCargaEvtDV: " + log;
                    IntegracaoDetalhaCustoModel.atualizaComplemento(numeroUnicoIntegracao, "E");//erro
                }
            }
        } catch (Exception e) {
            throw new Exception("Erro ao executar procedure PrcContrInsCargaEvtDV: " + e);
        } finally {
            super.finalizar();
        }
        return executado;
    ...
    private ProcedureCaller getProcedureCaller(JdbcWrapper jdbc, Map<String, String> parametrosExecutacao) throws Exception {
        ProcedureCaller caller = new ProcedureCaller("CONTR_INS_CARGA_EVT_DV");
