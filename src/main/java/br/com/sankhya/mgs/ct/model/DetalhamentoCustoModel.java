...
public class DetalhamentoCustoModel {
    ...
    public void validaDadosInsert() throws Exception {
        ...
        if( vo.asBigDecimal("CODTIPOPOSTO") != null
            && vo.asBigDecimal("CODSERVMATERIAL") != null ){
            ErroUtils.disparaErro("Fineza verificar o posto e o serviço não podem estar preenchidos no mesmo lançamento!");
        }
    ...
    public void opcaoCalculaTaxa() throws Exception{
        NativeSqlDecorator calculandoTaxaSQL = new NativeSqlDecorator("select NVL( Mgstctcontratotaxa.Ativo, 'N' ) calcula, Mgstctcontratotaxa.Vlrtaxa , Mgstctlocaltipofat.Nulocaltipofat\n" +
                ...
                "AND NVL( Mgstctcontratotaxa.Ativo, 'N' ) = 'N' "); //quando o status for N = Não calcula na conta, S = Sim calcula na planilha
    ...
    */
    ...
    private void calculaTaxaManual() throws Exception {
        ...
        if( nativeSqlDecorator.proximo() ){
            valortotal = nativeSqlDecorator.getValorBigDecimal("VALOR_GERAL");
            taxa = nativeSqlDecorator.getValorBigDecimal("TAXA_ADM");
            valorUnitario = nativeSqlDecorator.getValorBigDecimal("VALOR");
        }
        valorUnitarioTemporario = valorUnitario;
